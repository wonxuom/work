...
export default function App() {
  ...
  const refreshNotes = async (search = "") => { try { setNotes(await api.listNotes(search)); } catch (e) { console.error(e); } };
  ...
  const onShortcut = (event: ShortcutEvent) => { if (event.state === "Pressed") void openCapture(); };

  useEffect(() => {
    void refreshNotes(); void refreshToday();
    if (isDesktop) { void api.getSettings().then((saved) => { setSettings(saved); register(saved.shortcut, onShortcut).catch(console.error); }); }
    const stop = isDesktop ? listen("note-created", () => { void refreshNotes(); void refreshToday(); }) : Promise.resolve(() => {});
    const focus = isDesktop ? getCurrentWindow().listen("tauri://focus", () => { void refreshNotes(view === "search" ? query : ""); void refreshToday(); }) : Promise.resolve(() => {});
    return () => { void stop.then((fn) => fn()); void focus.then((fn) => fn()); };
  }, []);
  useEffect(() => { document.documentElement.dataset.theme = settings.theme; }, [settings.theme]);
  useEffect(() => { if (view === "timeline") void refreshNotes(); if (view === "search") void refreshNotes(query); if (view === "today") void refreshToday(); if (view === "weekly") void api.weeklySummary().then(setWeekly).catch(() => {}); }, [view]);
  const saveSettings = async () => { await api.saveSettings(settings); await unregisterAll(); await register(settings.shortcut, onShortcut); };
  ...
  return <div className="app-shell">
    <aside className="sidebar" data-tauri-drag-region><button className="brand" onClick={() => setView("today")}>Trace</button><nav>{nav.map((item) => <button key={item.id} className={view === item.id ? "nav-item active" : "nav-item"} onClick={() => setView(item.id)}>{item.label}</button>)}</nav><button className="capture-trigger" onClick={openCapture}><span>빠른 입력</span><kbd>⌘ ;</kbd></button></aside>
...
function Header({ title, meta, action }: { title: string; meta?: string; action?: React.ReactNode }) { return <header className="page-header" data-tauri-drag-region><div data-tauri-drag-region><h1 data-tauri-drag-region>{title}</h1>{meta && <p data-tauri-drag-region>{meta}</p>}</div>{action}</header>; }
...
function Today({ plans, summary, onRefresh }: { plans: Plan[]; summary: Summary | null; onRefresh: () => Promise<void> }) {
  ...
  const save = async (plan: Plan, nextContent: string, nextMemo: string) => { await api.updatePlan(plan.id, nextContent, nextMemo); await onRefresh(); };
  ...
  return <div className="today-layout"><section className="panel plan-panel"><div className="section-title"><h2>오늘 계획</h2><span>{plans.filter((p) => p.completed).length}/{plans.length}</span></div>{(["Must", "Should", "Could"] as const).map((level) => <div className="priority-group" key={level}><h3>{level}</h3>{plans.filter((p) => p.priority === level).map((plan) => <PlanRow key={plan.id} plan={plan} onSave={save} onToggle={toggle} onRemove={remove} />)}</div>)}<form className="plan-add" onSubmit={add}><select value={priority} onChange={(e) => setPriority(e.target.value as Plan["priority"])}><option>Must</option><option>Should</option><option>Could</option></select><input value={content} onChange={(e) => setContent(e.target.value)} placeholder="계획 추가" /><button>추가</button></form></section><DailyRecord summary={summary} /></div>;
}
function PlanRow({ plan, onSave, onToggle, onRemove }: { plan: Plan; onSave: (plan: Plan, content: string, memo: string) => Promise<void>; onToggle: (plan: Plan) => Promise<void>; onRemove: (id: number) => Promise<void> }) {
  const [content, setContent] = useState(plan.content); const [memo, setMemo] = useState(plan.memo);
  useEffect(() => { setContent(plan.content); setMemo(plan.memo); }, [plan.id, plan.content, plan.memo]);
  const save = async () => { const nextContent = content.trim(); const nextMemo = memo.trim(); if (!nextContent) { setContent(plan.content); return; } if (nextContent !== plan.content || nextMemo !== plan.memo) await onSave(plan, nextContent, nextMemo); };
  return <div className="plan-row"><input type="checkbox" checked={plan.completed} onChange={() => onToggle(plan)} /><div className="plan-edit"><input className={plan.completed ? "done" : ""} value={content} onChange={(e) => setContent(e.target.value)} onBlur={save} onKeyDown={(e) => { if (e.key === "Enter") e.currentTarget.blur(); }} aria-label="계획 수정" /><textarea value={memo} onChange={(e) => setMemo(e.target.value)} onBlur={save} rows={1} placeholder="메모" aria-label="계획 메모" /></div><button type="button" onClick={() => onRemove(plan.id)} aria-label="계획 삭제">×</button></div>;
}
